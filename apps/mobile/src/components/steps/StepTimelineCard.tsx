    ...
    return (
        ...
        >
            ...
            <View className="items-center mr-4 w-10">
                ...
                {!isLast && (
                    <View
                        className={`absolute top-10 bottom-[-24] w-0.5 ${isCompleted ? 'bg-success-500/50' : 'bg-surface-700/30'
                            }`}
                    />
                )}
                ...
                {/* Step Node */}
                <View
                    className={`w-10 h-10 rounded-full items-center justify-center border-2 z-10 ${isCurrent
            ...
            <View className="flex-1 pb-6">
                ...
                >
                    <BlurView
                        ...
                        tint="dark"
                        className={`p-5 border ${isCurrent
                                ? 'border-primary-500/30 bg-primary-500/5'
                                : 'border-white/5 bg-navy-800/40'
                            }`}
                    >
                        <View className="flex-row justify-between items-start mb-2">
                            <Text className={`text-base font-bold ${isCurrent ? 'text-white' : 'text-surface-300'
                                }`}>
                        ...
                        <Text className={`text-sm leading-5 ${isLocked ? 'text-surface-600' : 'text-surface-400'
                            }`}>
                            {step.description}
                        </Text>
                        ...
                        {!isLocked && (
                            <View className="flex-row items-center mt-4">
                                <Text className={`text-xs font-semibold mr-2 ${isCurrent ? 'text-primary-400' : 'text-surface-500'
                                    }`}>
