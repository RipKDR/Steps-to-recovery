...
export function useSponsorships() {
  ...
  const sendRequest = useCallback(async (sponsorEmail: string): Promise<void> => {
  ...
  const acceptRequest = useCallback(async (sponsorshipId: string): Promise<void> => {
    try {
  ...
  const declineRequest = useCallback(async (sponsorshipId: string): Promise<void> => {
    try {
      const { error } = await supabase
        .from('sponsorships')
  ...
  const removeSponsor = useCallback(async (sponsorshipId: string): Promise<void> => {
    try {
      const { error } = await supabase
        .from('sponsorships')
        .delete()
  ...
  // Filter helpers
  const mySponsor = sponsorships.find(
    s => s.sponsee_id === user?.id && s.status === 'accepted'
  );
